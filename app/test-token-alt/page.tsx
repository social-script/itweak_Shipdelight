...
export default function TestTokenAlt() {
  ...
  const generateToken = async () => {
    ...
    try {
      const response = await fetch('/api/shipdelight/test-token-alt');
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-gray-50">
      <Card className="w-full max-w-3xl shadow-md">
        <CardHeader>
          <CardTitle className="text-center">Alternative Shipdelight Token Test</CardTitle>
        </CardHeader>
        
        <CardContent className="space-y-4">
          <div className="text-center mb-4">
            <p>This test tries both POST with query parameter and POST with body parameter approaches.</p>
          </div>
          ...
          {tokenData && (
            <div className="mt-4 space-y-4">
              <div className="bg-green-50 border border-green-200 text-green-700 p-4 rounded-md">
                {tokenData.success ? `Token generated successfully using ${tokenData.approach} approach!` : 'Failed to generate token'}
              </div>
